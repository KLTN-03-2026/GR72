...
export class AdminMealTemplateService {
  ...
  async findAll(query: MealTemplateQueryDto) {
    ...
    if (query.trangThai)
      where.trang_thai = query.trangThai as MealTemplateStatus;
    if (query.loaiMucTieu)
      where.loai_muc_tieu_phu_hop = query.loaiMucTieu as any;
  ...
  private toPublic(e: ThucDonMauEntity) {
    return {
      ...
      nguoi_tao: e.nguoi_tao
        ? {
            id: e.nguoi_tao.id,
            ho_ten: e.nguoi_tao.ho_ten,
            email: e.nguoi_tao.email,
          }
        : null,
      tieu_de: e.tieu_de,
      mo_ta: e.mo_ta,
      ...
      chi_tiet:
        e.chi_tiet?.map((ct: ChiTietThucDonMauEntity) => ({
          id: ct.id,
          ngay_so: ct.ngay_so,
          loai_bua_an: ct.loai_bua_an,
          cong_thuc_id: ct.cong_thuc_id,
