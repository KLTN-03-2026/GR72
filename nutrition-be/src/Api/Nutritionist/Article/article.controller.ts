import { BadRequestException, Body, Controller, Delete, Get, NotFoundException, Param, ParseIntPipe, Patch, Post, Query, Req } from '@nestjs/common';
...
export class NutritionistArticleController {
  ...
  async findAll(@Req() req: any, @Query('trangThai') trangThai?: string, @Query('page') page = '1', @Query('limit') limit = '10') {
    ...
    const where: FindOptionsWhere<BaiVietEntity> = { tac_gia_id: userId, xoa_luc: IsNull() };
    if (trangThai) where.trang_thai = trangThai as ArticleStatus;

    const [items, total] = await this.repo.findAndCount({ where, order: { tao_luc: 'DESC' }, skip: (p - 1) * l, take: l });
    return { success: true, data: { items: items.map((i) => this.toPublic(i)), pagination: { page: p, limit: l, total } } };
  }

  @Get(':id')
  async findOne(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    const entity = await this.repo.findOne({ where: { id, tac_gia_id: req.user?.sub, xoa_luc: IsNull() } });
  ...
  async create(@Body() dto: CreateArticleDto, @Req() req: any) {
    ...
    return { success: true, message: 'Tao bai viet thanh cong', data: this.toPublic(saved) };
  }

  @Patch(':id')
  async update(@Param('id', ParseIntPipe) id: number, @Body() dto: UpdateArticleDto, @Req() req: any) {
    const entity = await this.repo.findOne({ where: { id, tac_gia_id: req.user?.sub, xoa_luc: IsNull() } });
    if (!entity) throw new NotFoundException('Bai viet khong ton tai');

    if (dto.tieuDe !== undefined) entity.tieu_de = dto.tieuDe.trim();
    if (dto.noiDung !== undefined) entity.noi_dung = dto.noiDung;
    if (dto.danhMuc !== undefined) entity.danh_muc = dto.danhMuc?.trim() || null;
    if (dto.theGan !== undefined) entity.the_gan = dto.theGan;
    if (dto.tomTat !== undefined) entity.tom_tat = dto.tomTat?.trim() || null;
    if (dto.anhDaiDienUrl !== undefined) entity.anh_dai_dien_url = dto.anhDaiDienUrl?.trim() || null;
    if (dto.huongDanAi !== undefined) entity.huong_dan_ai = dto.huongDanAi ?? null;
    entity.cap_nhat_luc = new Date();

    await this.repo.save(entity);
    return { success: true, message: 'Cap nhat bai viet thanh cong', data: this.toPublic(entity) };
  }
  ...
  async publish(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    ...
    entity.cap_nhat_luc = new Date();
    await this.repo.save(entity);
    return { success: true, message: 'Xuat ban thanh cong', data: this.toPublic(entity) };
  }
  ...
  async archive(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    ...
    entity.cap_nhat_luc = new Date();
    await this.repo.save(entity);
    return { success: true, message: 'Luu tru thanh cong', data: this.toPublic(entity) };
  }
  ...
  private toPublic(e: BaiVietEntity) {
    return {
      id: e.id, tac_gia_id: e.tac_gia_id, tieu_de: e.tieu_de, slug: e.slug,
      danh_muc: e.danh_muc, the_gan: e.the_gan, tom_tat: e.tom_tat,
      noi_dung: e.noi_dung, anh_dai_dien_url: e.anh_dai_dien_url,
      huong_dan_ai: e.huong_dan_ai, trang_thai: e.trang_thai,
