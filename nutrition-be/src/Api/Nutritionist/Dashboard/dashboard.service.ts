...
export class NutritionistDashboardService {
  constructor(
    ...
    private readonly mealRepo: Repository<ThucDonMauEntity>,
  ...
  async getSummary(userId: number) {
    const baseArticle: FindOptionsWhere<BaiVietEntity> = {
      tac_gia_id: userId,
      xoa_luc: IsNull(),
    };
    const baseRecipe: FindOptionsWhere<CongThucEntity> = {
      tao_boi: userId,
      xoa_luc: IsNull(),
    };
    const baseMeal: FindOptionsWhere<ThucDonMauEntity> = {
      tao_boi: userId,
      xoa_luc: IsNull(),
    };
    const baseReview: FindOptionsWhere<YeuCauDuyetThucPhamEntity> = {
      de_xuat_boi: userId,
    };
    ...
    const [
      ...
      this.reviewRepo.count({
        where: { ...baseReview, trang_thai: 'cho_duyet' },
      }),
      this.reviewRepo.count({
        where: { ...baseReview, trang_thai: 'da_duyet' },
      }),
      this.reviewRepo.count({
        where: { ...baseReview, trang_thai: 'tu_choi' },
      }),
