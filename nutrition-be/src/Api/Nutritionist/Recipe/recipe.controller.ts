...
export class NutritionistRecipeController {
  constructor(
    ...
    private readonly repo: Repository<CongThucEntity>,
    @InjectRepository(ThanhPhanCongThucEntity)
    private readonly ingredientRepo: Repository<ThanhPhanCongThucEntity>,
  ) {}
  ...
  async findAll(
    ...
    const where: FindOptionsWhere<CongThucEntity> = {
      tao_boi: userId,
      xoa_luc: IsNull(),
    };
  ...
  async findOne(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    const entity = await this.repo.findOne({
      where: { id, tao_boi: req.user?.sub, xoa_luc: IsNull() },
      relations: ['thanh_phan'],
    });
  ...
  async create(@Body() dto: CreateRecipeDto, @Req() req: any) {
    ...
    const entity = this.repo.create({
      tao_boi: req.user?.sub,
      ten: dto.ten.trim(),
      slug,
      mo_ta: dto.moTa?.trim() || null,
      huong_dan: dto.huongDan || null,
    ...
    return {
      success: true,
      message: 'Tao cong thuc thanh cong',
  ...
  async update(
    ...
    if (dto.soKhauPhan !== undefined)
      entity.so_khau_phan = dto.soKhauPhan ?? null;
    if (dto.trangThai !== undefined)
      entity.trang_thai = dto.trangThai as RecipeStatus;
  ...
  private toPublic(e: CongThucEntity) {
    return {
      ...
      slug: e.slug,
      mo_ta: e.mo_ta,
      huong_dan: e.huong_dan,
      so_khau_phan: e.so_khau_phan,
      ...
      thanh_phan:
        e.thanh_phan?.map((tp) => ({
          id: tp.id,
          thuc_pham_id: tp.thuc_pham_id,
          so_luong: Number(tp.so_luong),
          don_vi: tp.don_vi,
        })) ?? [],
