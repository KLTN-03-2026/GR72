...
export default function AdminNotificationsPage() {
  ...
  return (
    <>
      <PageHeader eyebrow='Trung tâm thông báo' title='Quản lý thông báo' description='Theo dõi thông báo hệ thống, lọc theo người nhận/trạng thái/loại và tạo thông báo thủ công khi cần.' action={<ActionButton tone='accent' onClick={openCreate}>Tạo thông báo</ActionButton>} />
      ...
      <Panel title='Danh sách thông báo' description='Danh sách dạng bảng để admin dễ kiểm tra thông báo đã gửi và trạng thái đọc.'>
        <Toolbar>
          ...
          <select className={inputClass} value={type} onChange={(e) => setType(e.target.value)}><option value=''>Tất cả loại</option>{['he_thong','booking','payment','review','commission','complaint'].map((item) => <option key={item} value={item}>{NOTIFICATION_TYPE_LABELS[item]}</option>)}</select>
