...
function SidebarProvider({
  ...
  React.useEffect(() => {
    ...
    const savedOpen = document.cookie
      .split('; ')
      .find((cookie) => cookie.startsWith(`${SIDEBAR_COOKIE_NAME}=`))
      ?.split('=')[1]

    if (savedOpen) {
      _setOpen(savedOpen !== 'false')
    }
  }, [openProp])
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden sidebar-scrollbar-hidden hover:sidebar-scrollbar-hover focus-within:sidebar-scrollbar-hover group-data-[collapsible=icon]:overflow-hidden',
        className
      )}
      {...props}
    />
