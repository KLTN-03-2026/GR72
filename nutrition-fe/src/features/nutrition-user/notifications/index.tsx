...
} from '@/services/user-notifications/api'
...
export function NutritionUserNotifications() {
  ...
  const handleMarkAllRead = useCallback(async () => {
    const unreadIds = items.filter((item) => item.trang_thai === 'chua_doc').map((item) => item.id)
    if (!unreadIds.length) return

    setMarkingAll(true)
    try {
      await Promise.all(unreadIds.map((id) => markUserNotificationRead(id)))
      toast.success('Đã đánh dấu tất cả đã đọc')
      await loadData()
    } catch (e) {
      toast.error(e instanceof ApiError ? e.message : 'Không thể cập nhật toàn bộ thông báo')
    } finally {
      setMarkingAll(false)
    }
  }, [items, loadData])
  ...
  const unreadCount = useMemo(
    () => items.filter((item) => item.trang_thai === 'chua_doc').length,
    [items],
  )
  ...
  const columns = useMemo<ColumnDef<Notification>[]>(() => [
    {
      ...
      header: 'Tiêu đề',
      cell: ({ row }) => (
        <button className='w-full text-left' onClick={() => void handleRowClick(row.original)}>
    ...
    },
    {
      accessorKey: 'loai',
      header: 'Loại',
      cell: ({ row }) => <Badge variant='outline' className='text-xs'>{row.original.loai}</Badge>,
    },
    {
  ...
  ], [handleMarkRead, handleRowClick, markingId, router])
  ...
  return (
    <>
      ...
      <Main className='flex flex-1 flex-col gap-5'>
        <PageHeading
          title='Thông báo'
          description='Theo dõi booking, thanh toán, hoàn tiền và các cập nhật quan trọng dành cho bạn.'
        />
        ...
        <div className='space-y-4'>
          <div className='flex flex-wrap items-center gap-3'>
            ...
            <div className='ml-auto'>
              <Button variant='outline' className='rounded-sm' onClick={() => void handleMarkAllRead()} disabled={markingAll || unreadCount === 0}>
